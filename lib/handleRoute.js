...
export default routeHandler => (req, res) => Promise.resolve()
  ...
  .then(handlerResults => {
    ...
    // Finds the build that matches the request version from the URL
    let latestVersion = handlerResults.version.split(',').find((v) => {
      return parseInt(reqVersion, 10) === parseInt(v.split('.')[2]);
    });

    const url = `${handlerResults.url}Backbone_${latestVersion}.cyacd`;
    res.send({ version: latestVersion, url: url });
  })
  .catch((err) => {
