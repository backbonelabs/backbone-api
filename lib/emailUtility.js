 ...
 * @const {String} fromAddress Default email address to use as the sender
 */
const apiKey = process.env.BL_MAILGUN_API;
const domain = process.env.BL_MAILGUN_DOMAIN;
const mailer = mailgun({ apiKey, domain });
const fromAddress = `Backbone <hello@${process.env.BL_MAILGUN_DOMAIN}>`;
...
const sendEmail = (data, email) => new Promise((resolve, reject) => {
  const packagedData = Object.assign({ to: email, from: fromAddress }, data);
