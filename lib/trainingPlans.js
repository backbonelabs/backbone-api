    ...
    level.forEach((session) => {
      session.forEach((sessionItem) => {
        ...
        if (!sessionItem.title) {
          ...
          // so generate the title based on the muscle group and type
          const {
            muscleGroups,
            workoutTypes,
          } = constants;
          const title = [];
          if (workout.muscle && muscleGroups[workout.muscle]) {
            title.push(muscleGroups[workout.muscle]);
          }
          if (workout.type && workoutTypes[workout.type]) {
            title.push(workoutTypes[workout.type]);
          }
...
/**
 *
 * @param {Object} Full training plans
 * @return {Object[]} List of default training plan(s)
 */
const getDefaultTrainingPlanIds = (plans) => {
